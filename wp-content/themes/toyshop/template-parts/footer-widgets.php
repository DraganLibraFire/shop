...
$helper =0;
	for( $i=0; $i < $footer_layout_number; $i++ ){
		...
		if(array_key_exists($i, $footer_elem_class)){?>
			<div class="col-md-<?php echo $footer_elem_class[$i]?>"><?php dynamic_sidebar($footer_slug);?></div>
			<?php 
		}else { 
			if(!array_key_exists($helper, $footer_elem_class)){ $helper=0;}
			?>
			<div class="col-md-<?php echo $footer_elem_class[$helper]?>"><?php dynamic_sidebar($footer_slug);?></div>
			<?php 
			$helper++;
		}	
	}	
